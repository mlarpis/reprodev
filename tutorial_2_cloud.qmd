---
title: "Tutorial 2 (Posit Cloud)"
subtitle: "[`git`](https://git-scm.com) and [`GitHub`](https://github.com) - creating a visible portfolio"
editor: source
---

This tutorial starts where we left off in [tutorial 1](tutorial_1.html). If you didn't finish that tutorial,
you can use [this repository](https://github.com/mlarpis/reprodev-exercise_quarto) as the base for Tutorial 2. 
...
First, we will create a new repository on `GitHub`. Go to [https://www.github.com](https://www.github.com) and log into your account. 

- Click on the `Repositories` tab and then on the green `New` button
- Give your repo a name and a description
- Choose `Public` as the visibility
- Choose initialize this repository with a README
- Choose MIT License
- Choose `R` for _Git Ignore_ 

![](tutorial_2_cloud_img/repo.gif)

2. Now publish the repository to `GitHub` by clicking _Create Repository_ and admire your new repository. 

## 2. Add content
This one is easy!

Add the content from Tutorial 1 to the folder that you just created. If you don't have a record of it, you can use [this repository](https://github.com/mlarpis/reprodev-exercise_quarto) as the base. 

You can overwrite the `README.md` file with the one from Tutorial 1. Add all files from the your repo, except for the _renv_ folder (if you do you will get a complaint about the number of files). There is no need to overwrite the `.gitignore` and `LICENSE` files, if relevant. 
...
Make sure that all folders in your repo are detected by `git`. You'll get an overview of the added files and folders. If you see that some folders are not detected, or if you have forgotten some files, you can add them manually by clicking on the _+ Add_ button.
:::

After making sure all changes to the folder are detected, create a commit message (`initial commit` is conventional) and click _Commit to main_.

![](tutorial_2_cloud_img/upload.gif)
...
![](tutorial_2_cloud_img/fork.png){width=100%}

2. Clone the repository to a new RStudio project on `posit.cloud`

![](tutorial_2_cloud_img/project.gif)

## 3. Create a new branch
Never work on the main branch. Always create a new branch for your work. With Posit Cloud, we need to first create an access token to communicate with `GitHub`.

1. Go to your `GitHub` account and click on your profile picture in the top right corner. Choose _Settings_ and then _Developer settings_. There you can create a token. 

- A classic token will suffice
- The token needs to have the `repo` scope
- Choose an expiration date that you are comfortable with

Don't forget to copy the token, once you are done. It will only be shown once.

![](tutorial_2_cloud_img/token.gif)

2. Take your copied token and go to Posit Cloud. Click on the _new branch_ icon to create a new branch. 

 - `dev` is a common name for a development branch
 - you'll have to authenticate with your `GitHub` account
 - use your `GitHub` username as the username
 - use the copied token as the password

![](tutorial_2_cloud_img/branch.gif)

3. Now you can start working on the repository.
...
3. Commit the changes to the repository (commit button) and push the changes to your fork of the repository on `GitHub` by clicking the green up arrow.

## 5. Create a pull request
Now that you have contributed to the project and have made changes to the repository, you can create a pull request to the main repository. I will illustrate this process on [the course's `reprodev-exercise_quarto` repository](https://github.com/mlarpis/reprodev-exercise_quarto)
...
Go through the files and look at the code. 
