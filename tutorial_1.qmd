...
We will work in `R`, although the materials and flow would be easily applicable to `Python`. Because we are focusing on development in `R`, I will outline the `RStudio IDE` workflows. It is not hard to adapt these workflows to e.g. `VScode` or `Jupyter Lab`.

The materials are available [in this branch of the course repository](https://github.com/mlarpis/reprodev/tree/exercise_quarto). 
You can download the materials by following these steps:

1. Open the [`exercise_quarto` branch](https://github.com/mlarpis/reprodev/tree/exercise_quarto).
